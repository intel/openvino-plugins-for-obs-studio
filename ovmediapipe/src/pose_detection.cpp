...
namespace ovmediapipe
{
    ...
    PoseDetection::PoseDetection(std::string model_xml_path, std::string device, ov::Core& core)
    {
        ...
        //prepare inputs / outputs
        {
            if (model->inputs().size() != 1) {
                throw std::logic_error("PoseDetection model topology should have only 1 input");
            }
    ...
    void PoseDetection::Run(const cv::Mat& frameBGR, std::vector<DetectedObject>& results)
    {
    ...
    void PoseDetection::preprocess(const cv::Mat& img, std::array<float, 16>& transform_matrix)
    {
    ...
    static void DecodeBoxes(const float* raw_boxes, const std::vector<Anchor>& anchors, std::vector<float>* boxes, int num_boxes_, int num_coords_)
    {
        const int box_coord_offset = 0;
        const bool reverse_output_order = true;
        const float x_scale = 224.f;
        const float y_scale = 224.f;
        const float h_scale = 224.f;
        const float w_scale = 224.f;
    ...
    static void ConvertToDetections(const float* detection_boxes, const float* detection_scores, const int* detection_classes, std::vector<DetectedObject>& output_detections, int num_boxes_, int num_coords_, size_t max_results = -1)
    {
    ...
    void PoseDetection::postprocess(const cv::Mat& frameBGR, std::array<float, 16>& transform_matrix, std::vector<DetectedObject>& results)
    {
        results.clear();
        ...
        const int num_boxes_ = maxProposalCount;
        const int num_classes_ = 1;
        const int num_coords_ = 12;
        ...
        DecodeBoxes(raw_boxes, anchors, &boxes, num_boxes_, num_coords_);
        ...
        for (size_t i = 0; i < results.size(); i++)
        {
            ...
            // recalculate ROI from the keypoints
            const float x_center =
                results[i].keypoints[start_keypoint_index].x *
                frameBGR.cols;
            const float y_center =
                results[i].keypoints[start_keypoint_index].y *
                frameBGR.rows;

            const float x_scale =
                results[i].keypoints[end_keypoint_index].x *
                frameBGR.cols;
            const float y_scale =
                results[i].keypoints[end_keypoint_index].y *
                frameBGR.rows;
            ...
            results[i].center = { x_center / frameBGR.cols, y_center / frameBGR.rows };
            results[i].width = box_size / frameBGR.cols;
            results[i].height = box_size / frameBGR.rows;
            results[i].bCenterValid = true;
        }
